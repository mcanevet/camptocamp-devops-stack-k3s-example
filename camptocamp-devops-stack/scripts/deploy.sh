...
if test "$(kubectl -n argocd get pods --selector 'app.kubernetes.io/name=argocd-server' --output=name|wc -l)" -eq 0; then
	helm dependency update camptocamp-devops-stack/argocd/argocd
	kubectl create namespace argocd || true
	helm template --include-crds argocd camptocamp-devops-stack/argocd/argocd --values "$ARTIFACTS_DIR/values.yaml" --set bootstrap=true --namespace argocd | kubectl "$KUBECTL_COMMAND" -n argocd -f -
